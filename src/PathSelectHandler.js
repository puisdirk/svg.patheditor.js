import { getWindow, List, Matrix } from '@svgdotjs/svg.js'
...
export class PathSelectHandler {
  ...
  drawControlPoints() {
    ...
    for (let i = 0, len = cpoints.length; i < len; ++i) {
      ...
      let controlPoint = this.drawPathSelectionControl(cpoints[i])
      controlPoint.addClass(cpoints[i].type).attr('segmentindex', cpoints[i].segmentIndex).attr('cpid', cpoints[i].cpid)
  ...
  getPathControlPoints() {
    ...
    segments.forEach(function (segment) {
      ...
      if (segment.command === 'smooth quadratic curveto') {
        ...
        if (
          prevSegment !== null &&
          (prevSegment.command === 'quadratic curveto' || prevSegment.command === 'smooth quadratic curveto')
        ) {
          controlPoints.push({
            type: _that.REFLECTED_CP,
            x: segment.x - (prevSegment.x1 - prevSegment.x0),
          ...
          controlPoints.push({
            type: _that.REFLECTED_STALK,
            subtype: 'x1y1refleft',
            xfrom: segment.x - (prevSegment.x1 - prevSegment.x0),
      ...
      if (segment.command === 'elliptical arc') {
        ...
        controlPoints.push({
          ...
          subtype: 'largesweep',
          pathstring: _that.getArcHelperPath(
            leftPoint,
            rightPoint,
            true,
            true,
  ...
  drawPathSelectionControl(pathSelectionControlInfo) {
    switch (pathSelectionControlInfo.type) {
      case this.XY_CP:
        return this.drawPathSelectionPoint(pathSelectionControlInfo.x, pathSelectionControlInfo.y).addClass(
          'svg_pathselect_control_point'
        )
      case this.X1Y1_CP:
        return this.drawPathSelectionHandle(pathSelectionControlInfo.x1, pathSelectionControlInfo.y1).addClass(
          'svg_pathselect_control_handle'
        )
      case this.X2Y2_CP:
        return this.drawPathSelectionHandle(pathSelectionControlInfo.x2, pathSelectionControlInfo.y2).addClass(
          'svg_pathselect_control_handle'
        )
      case this.STALK:
        return this.drawPathSelectionStalk(
          pathSelectionControlInfo.xfrom,
          pathSelectionControlInfo.yfrom,
          pathSelectionControlInfo.xto,
          pathSelectionControlInfo.yto
        )
          .addClass('svg_pathselect_control_stalk')
          .back()
      case this.REFLECTED_CP:
        return this.drawPathSelectionHandle(pathSelectionControlInfo.x, pathSelectionControlInfo.y)
          .addClass('svg_pathselect_control_handle')
          .addClass('reflected')
      ...
      case this.ARC_HELPER_PATH:
        return this.drawPathSelectionArcHelperPath(pathSelectionControlInfo.pathstring).addClass(
          'svg_pathselect_arc_helperpath'
        )
      case this.ARC_RX_CP:
      case this.ARC_RY_CP:
        return this.drawPathSelectionHandle(pathSelectionControlInfo.x, pathSelectionControlInfo.y).addClass(
          'svg_pathselect_control_handle'
        )
  ...
  updatePathPointSelection() {
    ...
    let _that = this
    this.selection.list.each(function () {
