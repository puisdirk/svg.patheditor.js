...
export class PathManipulator {
  ...
  init(options) {
    ...
    for (let i in this.el.manipulate.defaults) {
    ...
    this.el.on('controlpoint.pathmanipulator', function (e) {
      _this.handleControlPointEvent(e)
    }) // ControlPoint-Moving
    ...
    switch (controlpointType) {
          ...
          switch (segmentArr[this.parameters.segmentIndex][0]) {
            ...
            case 'M':
              ...
              if (segmentArr.length >= this.parameters.segmentIndex + 1) {
                let cmd = segmentArr[this.parameters.segmentIndex + 1][0]

                // TODO: need to do this for all commands that apply
                if (cmd === 'C') {
                  segmentArr[this.parameters.segmentIndex + 1][1] =
                    this.parameters.completeOriginalArray[this.parameters.segmentIndex + 1][1] + diffX
                  segmentArr[this.parameters.segmentIndex + 1][2] =
                    this.parameters.completeOriginalArray[this.parameters.segmentIndex + 1][2] + diffY
                }
              }
              break
            case 'l':
      ...
      case 'arc_rx_control_point':
        this.calc = function (diffX, diffY) {
          ...
          diffY = 0
          segmentArr[this.parameters.segmentIndex][1] = this.parameters.segmentInfo[1] - diffX + diffY
      ...
      case 'arc_rotation_control_point':
        this.calc = function (diffX, diffY) {
          ...
          diffY = 0
          segmentArr[this.parameters.segmentIndex][3] = diffX + diffY
